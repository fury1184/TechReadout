...
def _spec_summary(spec):
    """Human-readable one-liner of the type-specific specs."""
...
def export_excel():
    """Export Inventory, Hosts, and Hardware Specs to a styled .xlsx workbook."""
    import io
    ...
    def style_sheet(ws, headers, rows):
        ...
        for row in rows:
            ws.append(row)
        # Auto-size columns (capped so notes don't blow out the layout)
    ...
    summary_rows = [
        ...
        ('Inventory items (rows)', Inventory.query.count()),
        ('Inventory quantity (units)', int(total_qty)),
        ('Inventory value (excl. Sold/Dead)', float(inventory_value)),
        ('Hardware specs', HardwareSpec.query.count()),
        ('Hosts', Host.query.count()),
    ]
    summary.append(['TechReadout Export'])
    ...
    for row in summary.iter_rows(min_row=3, max_col=1):
        row[0].font = Font(bold=True)
    # Currency formatting for the value cell
    summary.cell(row=2 + 4, column=2).number_format = '"$"#,##0.00'

    # --- Inventory sheet ---
    ws_inv = wb.create_sheet('Inventory')
    inv_headers = ['ID', 'Name', 'Type', 'Status', 'Condition', 'Qty', 'Purchase Date',
                   'Purchase Price', 'Est.', 'Location', 'Assigned Host', 'Sale Date',
                   'Sale Price', 'Sold To', 'Notes']
    inv_rows = []
    for item in Inventory.query.order_by(Inventory.id).all():
        inv_rows.append([
            item.id,
            item.display_name,
            item.component_type.name if item.component_type else '',
            item.status,
            item.item_condition,
            item.quantity,
            ...
            'Yes' if item.price_is_estimate else '',
            item.location or '',
    ...
    style_sheet(ws_inv, inv_headers, inv_rows)
    for col in ('H', 'M'):  # price columns
        for cell in ws_inv[col][1:]:
            cell.number_format = '"$"#,##0.00'

    # --- Hosts sheet ---
    ws_hosts = wb.create_sheet('Hosts')
    host_headers = ['ID', 'Hostname', 'Status', 'Purpose', 'OS', 'IP Address',
                    'MAC Address', 'Components', 'Description']
    host_rows = []
    for host in Host.query.order_by(Host.hostname).all():
        host_rows.append([
            ...
            host.ip_address or '',
            host.mac_address or '',
            host.components.count(),
            host.description or '',
        ])
    style_sheet(ws_hosts, host_headers, host_rows)

    # --- Hardware Specs sheet ---
    ws_specs = wb.create_sheet('Hardware Specs')
    spec_headers = ['ID', 'Type', 'Manufacturer', 'Model', 'Specs', 'MSRP',
                    'Release Date', 'Source URL']
    spec_rows = []
    for spec in HardwareSpec.query.order_by(HardwareSpec.component_type_id, HardwareSpec.model).all():
        spec_rows.append([
            spec.id,
            spec.component_type.name if spec.component_type else '',
            spec.manufacturer or '',
            spec.model,
            _spec_summary(spec),
            float(spec.msrp) if spec.msrp is not None else None,
            spec.release_date.isoformat() if spec.release_date else '',
            spec.source_url or '',
        ])
    style_sheet(ws_specs, spec_headers, spec_rows)
    for cell in ws_specs['F'][1:]:
        cell.number_format = '"$"#,##0.00'
    ...
    wb.save(buf)
    buf.seek(0)
    filename = f"techreadout_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    return send_file(
        buf,
        as_attachment=True,
    ...
    for item_data in data.get('inventory', []):
        ...
        if existing:
            ...
            item = Inventory(
                ...
                status=item_data.get('status', 'Unverified'),
